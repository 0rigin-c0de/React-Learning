export function FractalHelpers(options) {
  return {
    ...getFractalBoxStyle(option.depth, options.maxDepth),
...
export function getFractalContainerStyle(options) {
  ...
  const scale =
    side === "right"
      ? Math.sqrt((size * heightFactor) ** 2 + (size * (0.5 + lean)) ** 2)
      : Math.sqrt((size * heightFactor) ** 2 + (size * (0.5 - lean)) ** 2);
...
export function shrink(
  startTime,
  startValue,
  duration = DefaultShrinkAnimationDuration,
  toValue = -3 / 5
) {
  return (
...
export function grow(
  startTime,
  startValue,
  duration = DefaultGrowAnimationDuration,
  toValue = 1
) {
  return (
    startValue + (1 - easeOut(startTime, duration)) * (toValue - startValue)
  );
}
